...
def noise_params(V_mean, V_std, dt=1.0, tau_m=10., C_m=250.):
    ...
    V_std = (V_std / np.sqrt(( 1 - math.exp(-dt/tau_m) ) / ( 1 + math.exp(-dt/tau_m) )))
    return V_mean, V_std
...
def simulate(mu, sigma, dt=1.0, tau_m=10., C_m=250., N=1000, t_max=100.):
    ...
    ng = nest.Create('noise_generator', params={'mean': mu*(resolution/tau_m), 'std': sigma*(resolution/tau_m), 'dt': dt})
    vm = nest.Create('voltmeter', params={'interval': resolution})
    nrns = nest.Create('iaf_simple', N, params={'E_L': 0., 'V_m': 0., 'V_th': 1e6,
                                                   'tau_m': tau_m, 'C_m': C_m})
