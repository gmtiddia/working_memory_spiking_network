...
def load_spike_data(overlap = False):
    if(overlap==False):
        srs = [np.loadtxt(data_path + "spikedata"+str(i)+".dat") for i in range(network_params["p"]+2)]
...
def testPS(panel="A"):
    ...
    data = {"panel": panel, "amplitude": network_params["A_eta"], "frequence": network_params["fr_eta"], "targeted_pop": targeted_sp}
    ...
    for sp in range(len(spike_times)):
        print(sp)
        if(sp<len(origin)):
            dum_origin = origin[sp]
        else:
            dum_origin = math.nan
        st = spike_times[sp]
        if(sp<network_params["p"]):
            print("Selective population {}".format(sp))
        elif(sp==network_params["p"]):
            print("Non-selective population")
        elif(sp==network_params["p"]+1):
        ...
        if(len(new_peak_times)!=len(peak_times)):
            print(len(new_peak_times), len(peak_times), origin)
            if(dum_origin!=math.nan and len(new_peak_times[new_peak_times<dum_origin])):
...
#data_path = "data_2A_notest_il_500/"

#data_path = "data_seq_test/"

# data_path = "data_2A_downscale_20/"

data_path = "data_2A_test_protocol/"
#data_path = "data_2A_thin_testsignal/"
...
#t, x, u, xstd, ustd, xsam, usam = get_stp_data_from_spikes(1.0, 0.19, sr0, 0, 0.1, 0.0, simulation_params["t_sim"])
#sys.exit()
figure = 2
...
if figure == 2:
    ...
    #save_stp_data(0)
    #t1, x1, u1, xstd1, ustd1, xsam1, usam1 = stp_data(0, sample=False)
    ...
    #if stp:
    #    save_stp_data(0)
    #    t, x, u, xstd, ustd, xsam, usam = stp_data(0, sample=False)
    #    #plt.plot(t,x)
    #    #plt.plot(t,u)
    #    #plt.show()
    #    figure2(stp = True, t=t, x=x, u=u, xstd=xstd, ustd=ustd, xs=xsam, us=usam, panel=panel)
    #else:
    #    figure2(stp = False, panel=panel)
