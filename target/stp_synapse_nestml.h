 ...
 *
 *  Generated from NESTML at time: 2024-10-23 08:04:26.264087
**/

#ifndef STP_SYNAPSE_NESTML_H
#define STP_SYNAPSE_NESTML_H
...
void register_stp_synapse_nestml( const std::string& name );

namespace stp_synapse_nestml_names
{
    const Name _u( "u" );
    const Name _x( "x" );
...
class stp_synapse_nestmlCommonSynapseProperties : public CommonSynapseProperties {
public:

    stp_synapse_nestmlCommonSynapseProperties()
...
class stp_synapse_nestml : public Connection< targetidentifierT >
{
private:
  double t_lastspike_;
  ...
  **/
  inline void
  update_internal_state_(double t_start, double timestep, const stp_synapse_nestmlCommonSynapseProperties& cp);

  void recompute_internal_variables();

public:
  // this line determines which common properties to use
  typedef stp_synapse_nestmlCommonSynapseProperties CommonPropertiesType;
  ...
  */
  stp_synapse_nestml( const stp_synapse_nestml& rhs );
  ...
  **/
  ...
  bool
  send( Event& e, const size_t tid, const stp_synapse_nestmlCommonSynapseProperties& cp )
  {
...
#ifdef DEBUG
    std::cout << "[synapse " << this << "] stp_synapse_nestml::send(): handling pre spike at t = " << __t_spike << std::endl;
#endif
...
void
register_stp_synapse_nestml( const std::string& name )
{
  nest::register_connection_model< stp_synapse_nestml >( name );
}
template < typename targetidentifierT >
constexpr ConnectionModelProperties stp_synapse_nestml< targetidentifierT >::properties;


template < typename targetidentifierT >
void
stp_synapse_nestml< targetidentifierT >::get_status( DictionaryDatum& __d ) const
{
  ...
  def< double >( __d, nest::stp_synapse_nestml_names::_w, P_.w );    // NEST special case for weight variable
  def< double >(__d, nest::stp_synapse_nestml_names::_U, get_U());
  def< double >(__d, nest::stp_synapse_nestml_names::_tau_rec, get_tau_rec());
  def< double >(__d, nest::stp_synapse_nestml_names::_tau_fac, get_tau_fac());
  def< double >( __d, names::delay, get_delay() );    // NEST special case for delay variable
  def<double>(__d, nest::stp_synapse_nestml_names::_delay, get_delay());
  def< double >(__d, nest::stp_synapse_nestml_names::_u, get_u());
  def< double >(__d, nest::stp_synapse_nestml_names::_x, get_x());
  def< double >(__d, nest::stp_synapse_nestml_names::_t_ls, get_t_ls());
}

template < typename targetidentifierT >
void
stp_synapse_nestml< targetidentifierT >::set_status( const DictionaryDatum& __d,
  ConnectorModel& cm )
{
  if (__d->known(nest::stp_synapse_nestml_names::_w) and __d->known(nest::names::weight))
  {
  ...
  double tmp_u = get_u();
  updateValue<double>(__d, nest::stp_synapse_nestml_names::_u, tmp_u);
  double tmp_x = get_x();
  updateValue<double>(__d, nest::stp_synapse_nestml_names::_x, tmp_x);
  double tmp_t_ls = get_t_ls();
  updateValue<double>(__d, nest::stp_synapse_nestml_names::_t_ls, tmp_t_ls);
  // special treatment of NEST weight
  double tmp_w = get_weight();
  if (__d->known(nest::stp_synapse_nestml_names::_w))
  {
    updateValue<double>(__d, nest::stp_synapse_nestml_names::_w, tmp_w);
  }
  ...
  updateValue<double>(__d, nest::stp_synapse_nestml_names::_tau_rec, tmp_tau_rec);
  double tmp_tau_fac = get_tau_fac();
  updateValue<double>(__d, nest::stp_synapse_nestml_names::_tau_fac, tmp_tau_fac);
  // special treatment of NEST delay
  double tmp_delay = get_delay();
  updateValue<double>(__d, nest::stp_synapse_nestml_names::_delay, tmp_delay);
...
**/
template < typename targetidentifierT >
void stp_synapse_nestml< targetidentifierT >::recompute_internal_variables()
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
**/
template < typename targetidentifierT >
stp_synapse_nestml< targetidentifierT >::stp_synapse_nestml() : ConnectionBase()
{
  const double __timestep = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the timestep() function
...
**/
template < typename targetidentifierT >
stp_synapse_nestml< targetidentifierT >::stp_synapse_nestml( const stp_synapse_nestml< targetidentifierT >& rhs )
: ConnectionBase( rhs )
{
  // parameters
  P_.U = rhs.P_.U;
...
template < typename targetidentifierT >
inline void
stp_synapse_nestml< targetidentifierT >::update_internal_state_(double t_start, double timestep, const stp_synapse_nestmlCommonSynapseProperties& cp)
{
    if (timestep < 1E-12)
    {
        std::cout << "[synapse " << this << "] stp_synapse_nestml: update_internal_state_() called with dt < 1E-12; skipping update" << std::endl;
        return;
    }
...
#ifdef DEBUG
    std::cout << "[synapse " << this << "] stp_synapse_nestml: Updating internal state: t_start = " << t_start << ", dt = " << timestep << std::endl;
#endif
...
#endif /* #ifndef STP_SYNAPSE_NESTML_H */