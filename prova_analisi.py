...
def figure2(stp = False, t=[], x=[], u=[], xstd=[], ustd=[], panel="A", data_path = "data/"):
    ...
    axs['upper_left'].plot([(h0[1][i]+h0[1][i+1])/2.0 for i in range(len(h0[0]))], fr0, color = "limegreen", alpha=1.0, label="Targeted pop.")  
    h1 = np.histogram(fr1, bins=bins)[0:2]
    fr1 = h1[0]/(binwidth/1000.0)/(network_params["N_exc"]*network_params["f"])
    axs['upper_left'].plot([(h1[1][i]+h1[1][i+1])/2.0 for i in range(len(h1[0]))], fr1, color = "black", alpha=1.0,label="Non-targeted pop. ")
    h2 = np.histogram(fr2, bins=bins)[0:2]
    """
    fr2 = h2[0]/(binwidth/1000.0)/(network_params["N_exc"]*network_params["f"])
    axs['upper_left'].plot([(h2[1][i]+h2[1][i+1])/2.0 for i in range(len(h2[0]))], fr2, color = "blue", alpha=0.5,label="Selective pop. 3")
    h3 = np.histogram(fr3, bins=bins)[0:2]
    fr3 = h3[0]/(binwidth/1000.0)/(network_params["N_exc"]*network_params["f"])
    axs['upper_left'].plot([(h3[1][i]+h3[1][i+1])/2.0 for i in range(len(h3[0]))], fr3, color = "red", alpha=0.5,label="Selective pop. 4")
    h4 = np.histogram(fr4, bins=bins)[0:2]
    fr4 = h4[0]/(binwidth/1000.0)/(network_params["N_exc"]*network_params["f"])
    axs['upper_left'].plot([(h4[1][i]+h4[1][i+1])/2.0 for i in range(len(h4[0]))], fr4, color = "orange", alpha=0.5,label="Selective pop. 5")
    axs['upper_left'].legend(loc='upper left',fontsize=13)
    """
    axs['upper_left'].legend(loc='upper left',fontsize=13)
    ...
    n = 7  # Mantieni 1 punto ogni 10
    axs['lower_left'].plot(sr0[::n,1], sr0[::n,0], '.', color="limegreen", label="Sel Pop {}".format(simulation_params["recording_params"]["pop_recorded"][0]))
    axs['lower_left'].plot(sr1[::n,1], sr1[::n,0] - [network_params["N_exc"] * network_params["f"] for i in sr1[::n,0]], '.', color="k", label="Sel Pop {}".format(simulation_params["recording_params"]["pop_recorded"][1]))
    ...
    axs['lower_left'].text(-0.1, 1.1, 'B', transform=axs['lower_left'].transAxes, fontsize=22, fontweight='bold', va='top')
    axs['upper_left'].text(-0.1, 1.1, 'A', transform=axs['upper_left'].transAxes, fontsize=22, fontweight='bold', va='top')
    axs['right'].text(-0.1, 1.05, 'C', transform=axs['right'].transAxes, fontsize=22, fontweight='bold', va='top')
    ...
    plt.savefig(data_path+"seed_10_fig2{panel}.png".format(panel=panel))
    #print("bro sono qui")
    plt.draw()
...
for i in range(0, n):
    ...
    plot_name = data_path + "panel" + str(figure) +"_" +"B.png"
    if figure == 4:
        if stp:
